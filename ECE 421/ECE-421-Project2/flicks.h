...
namespace util {
	...
	typedef std::chrono::duration<std::chrono::nanoseconds::rep, std::ratio<1, 705600000>> flicks;
...
//!
	...
	unsigned int flicks_per_second() {
		return std::chrono::duration_cast<flicks>(std::chrono::seconds{ 1 }).count();
	}
}  // namespace util